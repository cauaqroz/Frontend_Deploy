  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      if (Object.keys(modifiedUserData).length > 0) {
        const userResponse = await axios.patch(' https://backend-conecta-09de4578e9de.herokuapp.com/users/update', modifiedUserData, {
          headers: {
      ...
      if (isFreelancer) {
        ...
        if (Object.keys(modifiedFreelancerData).length > 0) {
          if (hasFreelancerProfile) {
            const freelancerResponse = await axios.patch(' https://backend-conecta-09de4578e9de.herokuapp.com/users/freelancer/update', modifiedFreelancerData, {
              headers: {
                'Content-Type': 'application/json',
                'userId': userId
              }
            ...
            const freelancerResponse = await axios.post(' https://backend-conecta-09de4578e9de.herokuapp.com/users/freelancer', freelancerData, {
              headers: {
                'Content-Type': 'application/json',
                'userId': userId
              }
