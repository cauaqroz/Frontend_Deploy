...
const Projetos = () => {
  ...
  useEffect(() => {
    if (selectedProjeto) {
      fetch(
        `${config.LocalApi}/projetos/${selectedProjeto.id}/pedidosParticipacao`
      )
        .then((response) => response.json())
        .then((ids) => {
          Promise.all(
            ids.map((id) =>
              fetch(`${config.LocalApi}/users/${id}`).then((res) => res.json())
            )
      ...
      if (
        selectedProjeto.approvedParticipants &&
        selectedProjeto.approvedParticipants.length > 0
      ) {
        Promise.all(
          selectedProjeto.approvedParticipants.map((id) =>
            fetch(`${config.LocalApi}/users/${id}`).then((res) => res.json())
          )
  ...
  const handleAprovar = (userId) => {
    if (selectedProjeto && user) {
      fetch(
        `${config.LocalApi}/projetos/${selectedProjeto.id}/aprovarUsuario`,
        {
          method: "POST",
          headers: {
  ...
  const handleNegar = (userId) => {
    if (selectedProjeto && user) {
      fetch(
        `${config.LocalApi}/projetos/${selectedProjeto.id}/negarSolicitacao/${userId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleUpdateProjeto = async (updatedProjeto) => {
    ...
    try {
      const response = await fetch(
        `${config.LocalApi}/projetos/${selectedProjeto.id}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const enviarMensagem = async () => {
    ...
    try {
      const response = await fetch(
        `${config.LocalApi}/chat/${selectedProjeto.chatId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      case "informacoes":
        return (
          <>
            ...
            <p>
              <strong>Tecnologia:</strong> {selectedProjeto.tecnologia}
            </p>
          ...
          <div className="canal-card">
            ...
            <div className="mensagens">
              {mensagens.map((mensagem) => (
                <div
                  key={mensagem.createdDate}
                  className={`mensagem ${
                    mensagem.sender === user.id
                      ? "mensagem-direita"
                      : "mensagem-esquerda"
                  }`}
                >
                  {mensagem.sender !== user.id && (
                    <p>
                      <strong>{mensagem.senderName}</strong>
                    </p>
                  )}
            ...
            >
              <div>
                ...
                <input
                  type="text"
                  name="titulo"
                  defaultValue={selectedProjeto.titulo}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  type="text"
                  name="tecnologia"
                  defaultValue={selectedProjeto.tecnologia}
                />
              </div>
  ...
  return (
    <div className="projects-page">
      ...
      <div className="container-projects">
        ...
        {loading ? (
          < >
            <Skeleton type="card" page="projetos" />
            <Skeleton type="card" page="projetos" />
            <Skeleton type="card" page="projetos" />
            </>
          ) : (
              ...
              {selectedProjeto ? (
                <div className="projects-detalhes full-screen">
                  ...
                  <div className="project-card">
                    <img
                      src={
                        selectedProjeto.capaUrl
                          ? `${config.LocalApi}/projetos/${selectedProjeto.id}/capa`
                          : defaultImage
                      }
                      alt="Capa do Projeto"
                    />
                  ...
                  <div className="tabs">
                    ...
                    <button
                      ...
                      className={activeTab === "canal" ? "active" : ""}
                    >
                      Canal
                    </button>
                    <button
                      ...
                      projetosCriados.map((projeto) => (
                        ...
                        >
                          <img
                            ...
                            }} // Adiciona fallback para imagem padrão
                          />
